<script lang="ts">
    ...
    const dropPanel = (ev: { target: { id: string } }) => {
        ...
        const draggedPanelIndex = panelForm.findIndex((panel) => panel.position === draggedPanel.position);
        const currentPanelIndex = panelForm.findIndex((panel) => panel.position === currentPanel.position);
